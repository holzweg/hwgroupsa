<?php
/**
 * File containing the hwGroupSaLanguageSwitcher class
 *
 */

/**
 * Utility class for transforming URLs between siteaccesses with group sa language.
 *
 * This class will generate URLs for various siteaccess, and translate
 * URL-aliases into other languages as necessary.
 */

class hwGroupSaLanguageSwitcher implements ezpLanguageSwitcherCapable
{
    protected $origUrl;
    ...
    public function destinationUrl()
    {
        ...
        }elseif( $GLOBALS['eZCurrentAccess']['type'] === eZSiteAccess::TYPE_HTTP_HOST_URI_GROUP) {
            $saArray = explode("/", $this->destinationSiteAccess);
            $finalUrl = $this->baseDestinationUrl . '/' . $saArray[1] . '/' . $urlAlias;
        }else{
    ...
    public function setDestinationSiteAccess( $saName )
    {
        ...
        if(isset($sa["sub"])) {
            // Set to real siteaccess
            $this->destinationSiteAccess = $sa["name"] . "/" . $saName;
        }else{
    ...
    public function process()
    {
        ...
        switch ( $GLOBALS['eZCurrentAccess']['type'] )
        {
            case eZSiteAccess::TYPE_URI:
                ...
                break;
            case eZSiteAccess::TYPE_HTTP_HOST_URI_GROUP:
                eZURI::transformURI( $host, true, 'full' );
                break;
    ...
    public static function setupTranslationSAList( $url = null )
    {
        ...
        foreach($siteAccess["subs"] as $subSiteAccessName) {
            $switchLanguageLink = "/switchlanguage/to/{$subSiteAccessName}/";
            if ( $url !== null && ( is_string( $url ) || is_numeric( $url ) ) )
            {
                $switchLanguageLink .= $url;
            }
            $translationName = $translationSiteAccesses[$subSiteAccessName];
            $ret[$subSiteAccessName] = array(
                'url' => $switchLanguageLink,
                'text' => $translationName,
                'locale' => eZSiteAccess::getIni( "$siteAccessName/$subSiteAccessName" )->variable( 'RegionalSettings', 'ContentObjectLocale' )
             );
        }
